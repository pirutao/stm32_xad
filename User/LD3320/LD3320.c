...
void send_voice_result(uint8 nAsrRes)
{
	uint8 voice_key_len;
	
	memset(&gvr.uart1_tx,0,sizeof(gvr.uart1_tx));
	gvr.uart1_tx.head1 = 0xeb;
	gvr.uart1_tx.head2 = 0x90;
	gvr.uart1_tx.type = VOICE_ALARM;
	gvr.uart1_tx.sn = nAsrRes;
	voice_key_len = strlen((char *)&gvr.voiceKey.key[nAsrRes-1]);
	memcpy(gvr.uart1_tx.voice_str,(char *)&gvr.voiceKey.key[nAsrRes-1],voice_key_len);
	USART_SendBuf((char *)&gvr.uart1_tx,voice_key_len+4);
	gvr.uart1_tx.type = 0;
}
///用户修改
void LD3320_loop(void)
{
	...
	switch(nAsrStatus)
	{
		...
		case LD_ASR_FOUNDOK:
				...
				if(nAsrRes != 49)//don,t send duty key ack
					send_voice_result(nAsrRes);
...
static uint8 LD_AsrAddFixed(void)
{
	...
	{			
		...
		for (nAsrAddLength=0; nAsrAddLength<KEY_LEN_MAX; nAsrAddLength++)
		{
			if (gvr.voiceKey.key[k][nAsrAddLength] == 0)
				break;
			LD_WriteReg(0x5, gvr.voiceKey.key[k][nAsrAddLength]);
		}
	...
	for (; k<gvr.duty_voiceKey.num; k++)//add duty voice keys
	{			
		if(LD_Check_ASRBusyFlag_b2() == 0)
		{
			flag = 0;
		...
		LD_WriteReg(0xc1, 49 );//id 49 is duty voice key
		LD_WriteReg(0xc3, 0);
		LD_WriteReg(0x08, 0x04);
		LD3320_delay(1);
		LD_WriteReg(0x08, 0x00);
		...
		for (nAsrAddLength=0; nAsrAddLength<KEY_LEN_MAX; nAsrAddLength++)
		{
			if (gvr.duty_voiceKey.key[k][nAsrAddLength] == 0)
				break;
			LD_WriteReg(0x5, gvr.duty_voiceKey.key[k][nAsrAddLength]);
		}
		LD_WriteReg(0xb9, nAsrAddLength);
		LD_WriteReg(0xb2, 0xff);
		LD_WriteReg(0x37, 0x04);
...
static uint8 LD_AsrAddFixed_bak(void)
{
	uint8 k, flag;
	uint8 nAsrAddLength;
